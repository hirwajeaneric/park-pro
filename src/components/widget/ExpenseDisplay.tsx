...
import { listBudgetsByPark, getMySubmittedExpenses } from '@/lib/api';
...
export default function ExpenseDisplay() {
    ...
    const { data: expenses = [], isLoading: isExpensesLoading } = useQuery({
        ...
        queryFn: () => getMySubmittedExpenses(selectedBudgetId!),
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => router.push(`/manager/expense/${row.original.id}`)}
                >
