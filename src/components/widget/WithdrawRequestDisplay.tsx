...
import { listBudgetsByPark, getMySubmittedWithdrawRequests } from '@/lib/api';
import { Budget, WithdrawRequest } from '@/types';

export default function WithdrawRequestDisplay() {
  ...
  const { data: withdrawRequests = [], isLoading: isRequestsLoading } = useQuery({
    ...
    queryFn: () => getMySubmittedWithdrawRequests(selectedBudgetId!),
  ...
  const columns: ColumnDef<WithdrawRequest>[] = [
    ...
    },
    {
      ...
      header: 'Status',
      cell: ({ row }) => (
        <Badge variant={row.getValue('status') === 'APPROVED' ? 'success' : 'secondary'}>
          {row.getValue('status')}
        </Badge>
      ),
    },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Button
          variant="ghost"
          size="sm"
          onClick={() => router.push(`/manager/withdraw-request/${row.original.id}`)}
        >
