...
import { OutstandingDonorResponse } from "@/types";
import { getTopDonorsByPark } from "@/lib/api";
...
export default async function Home() {
  ...
  // Fetch top donors for Loango National Park
  const parkId = process.env.NEXT_PUBLIC_PARK_ID;
  let donors: OutstandingDonorResponse[] = [];
  let error: string | null = null;

  try {
    donors = await getTopDonorsByPark(parkId);
  } catch (err) {
    error = err instanceof Error ? err.message : "Failed to fetch donor data";
  }
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="md:col-span-2">
              <p className="text-gray-700 mb-4">
                The first formal protected area was established in 1956, but it was not until 2002 that the park itself
            ...
            {Services.map((service, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div className="p-6 flex flex-col justify-between">
                  ...
                  <Link
                    href={service.slug}
                    className="w-full border flex text-center items-center justify-center font-semibold text-nowrap text-sm rounded-md py-2"
                  >
                    {service.name}
                  </Link>
                </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-3xl mx-auto">
            {error ? (
              ...
              <p className="text-center">No donors found for this park.</p>
            ) : (
              donors.map((donor) => (
                <div key={donor.donorId} className="border-b py-4 grid grid-cols-2">
                  <div className="font-medium">{donor.donorName}</div>
                  <div>
                    <div>{donor.totalDonationAmount} XAF</div>
                    <div className="text-sm text-gray-500">{donor.motiveForDonation || "No motive provided"}</div>
                  </div>
                </div>
              ))
            )}
